    ...
    <table class="table">
      ...
      <body>
        <?php
         ...
         $sql="SELECT `hotels`.`hotel_name`, `hotels`.`city`, `users`.`userName`, `users`.`email`, `bookings`.`book_id`, `bookings`.`checkin_date`, `bookings`.`checkout_date`
          FROM `hotels`, `users` , `bookings` 
         WHERE `bookings`.`user_id` = `users`.`user_id` AND bookings.hotel_name = hotels.hotel_name;";
